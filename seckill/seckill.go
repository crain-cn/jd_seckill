...
func (this *Seckill) SkuTitle() (string,error) {
	...
	resp,body,err:=req.SetUrl(fmt.Sprintf("https://item.m.jd.com/product/%s.html",skuId)).SetMethod("get").Send().End()
	...
	return strings.TrimSpace(doc.Find("#itemName").Text()),nil
}
...
func (this *Seckill) MakeReserve() {
	...
	skuId:=this.conf.Read("config","sku_id")
	req:=httpc.NewRequest(this.client)
	req.SetHeader("User-Agent",this.conf.Read("config","DEFAULT_USER_AGENT_APP"))
...
func (this *Seckill) SeckillInitInfo() (string,error) {
	...
	seckillNum:=this.conf.Read("config","seckill_num")
	req:=httpc.NewRequest(this.client)
	req.SetHeader("User-Agent",this.conf.Read("config","DEFAULT_USER_AGEN_APP"))
	req.SetHeader("Host","marathon.jd.com")
